...
@immutable
class Geometry {
  final SpaDetailLocation location;
  ...
  factory Geometry.fromMap(Map<String, dynamic> map) {
    return Geometry(
      location: SpaDetailLocation.fromMap(map['location']),
    );
  }
}

// Define the SpaDetailLocation model class
@immutable
class SpaDetailLocation {
  final double lat;
  final double lng;

  // Constructor
  const SpaDetailLocation({required this.lat, required this.lng});

  // Factory constructor to create an instance from a Map
  factory SpaDetailLocation.fromMap(Map<String, dynamic> map) {
    return SpaDetailLocation(
