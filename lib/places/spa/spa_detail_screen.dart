...
class SpaDetailScreen extends StatelessWidget {
  SpaDetailScreen({super.key, required this.placeArgument});

  static const PlaceCategoryEnum placeCategory = PlaceCategoryEnum.spa;
  ...
  Widget build(BuildContext context) {
    context
        .read<SpaDetailBloc>()
        .add(SpaDetailFetched(placeId: placeArgument.placeId));
    return BlocConsumer<SpaDetailBloc, SpaDetailState>(
      listener: (context, state) {
        if (state is SpaDetailSuccess) {}
        if (state is SpaDetailFailure) {
          ...
          return SafeArea(
            child: Scaffold(
              ...
              body: SizedBox(
                ...
                child: _body(context, state.spaDetail, hasCallSupport),
              ),
            ),
          );
        }
  ...
  Widget _spaPlaceImages(BuildContext context) {
    return SizedBox(
      width: double.maxFinite,
  ...
  Widget _spaPlaceName(BuildContext context, SpaDetailModel spaDetail) {
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        //         ? context.tr("spa_detail.place_claimed")
        //         : context.tr("spa_detail.place_not_claimed"),
  ...
  Widget _aboutTag(BuildContext context, SpaDetailModel spaDetail) {
    return Visibility(
      visible: spaDetail.types != null,
      child: SizedBox(
        width: double.maxFinite,
        child: Wrap(
          children: List<Widget>.generate(
            spaDetail.types == null ? 1 : spaDetail.types!.length,
            (index) => _tagView(
              context,
              label: spaDetail.types![index],
  ...
  Widget _aboutOperationalTime(BuildContext context, SpaDetailModel spaDetail) {
    return SizedBox(
      width: double.maxFinite,
      child: ExpansionTile(
        ...
        title: Column(
          ...
          children: [
            // Text(
            //   context.tr("spa_detail.about_open"),
            //   style: CustomTextStyles(context).bodyMedium_13,
            // ),
            _operationalDayHourView(context,
                DatetimeUtils.getTodayOperationalTime(), true, spaDetail),
          ],
        ),
        children: spaDetail.currentOpeningHours != null
            ? spaDetail.currentOpeningHours!.weekdayText.map((openingDay) {
                return _operationalDayHourView(
                    context,
                    spaDetail.currentOpeningHours!.weekdayText
                        .indexOf(openingDay),
                    false,
                    spaDetail);
              }).toList()
  ...
  Widget _operationalDayHourView(
      BuildContext context, int index, bool isTitle, SpaDetailModel spaDetail) {
    late DateTime openTime;
    late DateTime closeTime;
    if (spaDetail.currentOpeningHours != null) {
      String placeOpenCloseTime =
          spaDetail.currentOpeningHours!.weekdayText[index];
      ...
      child: Row(
        ...
        children: [
          SizedBox(
            ...
            child: Text(
              spaDetail.currentOpeningHours != null
  ...
  Widget _aboutFacilities(BuildContext context, SpaDetailModel spaDetail) {
    return SizedBox(
      width: double.maxFinite,
      child: ExpansionTile(
        ...
        dense: true,
        tilePadding: const EdgeInsets.symmetric(horizontal: 0.0),
        title: _facilitiesView(context, null, true, spaDetail),
        children: FilterDataset.facilitiesDataset.map(
          (filterData) {
            return _facilitiesView(context, filterData, false, spaDetail);
          },
        ).toList(),
      ),
    );
  ...
  Widget _facilitiesView(BuildContext context, FilterToggleModel? filterData,
      bool isTitle, SpaDetailModel spaDetail) {
    return SizedBox(
      width: double.maxFinite,
      child: Row(
        ...
        children: [
          ...
          SizedBox(
            width: isTitle ? 80.h : 120.h,
            child: Text(
              isTitle ? context.tr("spa_detail.facilities") : filterData!.label,
              style: CustomTextStyles(context).bodySmall12,
            ),
          ),
        ],
  ...
  Widget _aboutPrice(BuildContext context, SpaDetailModel spaDetail) {
    return SizedBox(
      width: double.maxFinite,
      child: ListTile(
        dense: true,
  ...
  Widget _spaPromo(BuildContext context, SpaDetailModel spaDetail) {
    return Visibility(
      visible: placeArgument.isMerchant,
      ...
      child: Container(
        ...
        child: Column(
          children: [
            CustomSectionHeader(
              label: context.tr("spa_detail.promo"),
              onPressed: () {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 5.h),
            Text(
              context.tr("spa_detail.promo_date",
  ...
  Widget _spaPlaceReviews(BuildContext context, SpaDetailModel spaDetail) {
    return Container(
      width: double.maxFinite,
      margin: EdgeInsets.symmetric(horizontal: 5.h),
      child: Column(
        children: [
          CustomSectionHeader(
            label: context.tr("spa_detail.review"),
            onPressed: () {
              Navigator.pushNamed(
                context,
          ...
          InkWell(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    context.tr("spa_detail.total_review", namedArgs: {
                      "total": spaDetail.reviews == null
                          ? "0"
                          : spaDetail.reviews!.length.toString(),
                    }),
                    style: CustomTextStyles(context).bodySmall12,
                    textAlign: TextAlign.center,
                  ),
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              spaDetail.rating.toString(),
                              style: CustomTextStyles(context).titleLarge_22,
                              textAlign: TextAlign.center,
                            ),
                            Text(
                              context.tr("spa_detail.avg_rating"),
